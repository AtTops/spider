...
public class SqlUtils {
    ...
    public static HashMap<String, String> getSpuCodeAndColor() {
        ...
        try {
            conn = DbFactory.getConnection();
            preStmt = conn.prepareStatement("select spu_code,color from product_spu_local;");
            rs = preStmt.executeQuery();
            while (rs.next()) {
    ...
    /**
     * 获取老的Spu_code、taobao_link
     *
     * @return hashmap
     */
    public static HashMap<String, String> getSpuCodeAndTbLinkOld() {
        System.out.println("-----------------------");
        HashMap<String, String> hashMap = new HashMap<>(512, 0.8f);
        Connection conn = null;
        PreparedStatement preStmt = null;
        ResultSet rs;
        // "select spu_code,taobao_link from product_spu_bak2;"
        try {
            conn = DbFactory.getConnection();
            preStmt = conn.prepareStatement("select spu_code,taobao_link from product_spu_local");
            rs = preStmt.executeQuery();
            while (rs.next()) {
                hashMap.put(rs.getString("spu_code"), rs.getString("taobao_link"));
            }
    ...
    public static void main(String[] args) {
        // 回滚线下
        SqlUtils.backOnlineOrLocal(true);
