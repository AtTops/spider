...
//    SELECT_SPU_TLINK("select spu_code,taobao_link,season from product_spu_local where season like '%2%' and vendor_id = 28", 2),
    SELECT_SPU_TLINK("select spu_code,taobao_link from product_spu where store_spu_code is not null and status = 1 and head_image !='' and LENGTH(taobao_link) > 40;", 2),
    ALL_SPU_TLINK("select spu_code,taobao_link from product_spu where LENGTH(taobao_link) > 40;", 12),
    SELECT_SPU_TLINK_BY_SPUCODE("select spu_code,taobao_link from product_spu_local where spucode = ?", 3),
//    SELECT_SPU_IMG_URL("SELECT spu_code,spider_imgs_all from product_spu_local where length(spider_imgs_all) > 4 ", 3),
    SPU_UPLOADED("SELECT spu_code from product_spu_local where length(spider_imgs_all) > 4 and upload_flag = '1'", 4),
    ...
    UPDATE_CDN_IMGS_ONLINE("update product_spu set head_image = ?,details_images = ? where spu_code = ?", 7),
    BACK_CDN_IMGS_LOCAL("update product_spu_local set head_image = ?,details_images = ? where spu_code = ?", 8),
